...
export const getThreads = async (category = null, pageSize = 10, lastDoc = null) => {
    try {
        ...
        if (category) {
            // If category is specified, only filter by category (no ordering to avoid index requirement)
            q = query(
            ...
            // If no category, only order by creation date
            q = query(
                collection(db, 'forum_threads'),
        ...
        // If category is specified, sort client-side by creation date
        if (category) {
            threads.sort((a, b) => {
...
export const getReplies = async (threadId) => {
    try {
        ...
        // Sort replies by createdAt in JavaScript (client-side) to avoid index requirement
        replies.sort((a, b) => {
            if (!a.createdAt) return 1;
            if (!b.createdAt) return -1;
...
export const searchThreads = async (searchTerm, category = null) => {
    try {
        // This is a simple client-side search. For production, consider a dedicated search service like Algolia.
        let q;
        ...
        if (category) {
            ...
            // If no category, get all threads (we'll search client-side)
